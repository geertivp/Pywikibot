...
codedoc = """
Replace a wrong Wikidata label.

Parameters:

    stdin: comma list of Qnumber, old label, new label
...
Author:
	...
	Geert Van Pamel, 2021-12-05, CC BY-SA 4.0, User:Geertivp
...
modnm = 'Pywikibot replace_label'   # Module name (using the Pywikibot package)
...
# (1) Update labels
            for lang in item.labels:
                if item.labels[lang] == name[0]:
                    item.labels[lang] = name[1]    # Set the label

# (2) Update aliases
            for lang in item.aliases:
                if name[0] in item.aliases[lang]:
                    for seq in item.aliases[lang]:
                        if seq == name[0]:
                            item.aliases[lang].remove(seq)
                            item.aliases[lang].append(name[1])    # Set the label
                            break
...
logger = logging.getLogger('replace_label')
...
# Connect to database
transcmt = 'Pwb replace label'	    	    # Wikidata transaction comment
