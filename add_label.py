            ...
            while commonscatqueue and (datetime.now() - lastwpedit).total_seconds() > 15.0:
                ...
                if page.text:
                    ...
                    if IMAGEPROP in item.claims and lang in item.labels:
                        ...
                        file_name_re = file_name[1].replace('(', '[(]').replace(')', '[)]')
                    ...
                    wpcommonscat_re = wpcommonscat.replace('(', '[(]').replace(')', '[)]')
                    categorytext = ''

                    # Add missing Commons Category
                    if (wpcommonscat
                            # Avoid duplicate Commons cat with human Infoboxes
                            and not (sitelang in veto_commonscat            ## Maybe too restrictive
                    ...
                    if item_instance == HUMANINSTANCE and sitelang not in veto_defaultsort:
                        try:
                            ...
                            if not re.search(sort_template[1:] + skip_defaultsort,
                                ...
                                if categorytext:
                                    categorytext += '\n'
                                categorytext += '{{' + sort_words[0] + sortorder + '}}'
                                pageupdated += ' ' + sort_words[0]
                                pywikibot.warning('Add {}{} to {}'
                                                  .format(sort_words[0], sortorder, sitelang))
                        except:
                    ...
                    wpcatpage_re = wpcatpage.replace('(', '[(]').replace(')', '[)]')
                    ...
                    if pageupdated != transcmt + ' Add':
                        ...
                        try:
                            ...
                            page.text = re.sub(r'{{DEFAULTSORT:', '{{' + sort_words[0], page.text)
...
infobox_localname = get_item_label_dict('Q15515987')
